...
def run_calculation(sub_list, MNI_mask, participant_tsv, temp_work, keep, failed_filepath): 
    ...
    for subject in sub_list:
        ...
        for session in os.listdir(subject):
            ...
            print("processing sub:",sub_id," ses:",session)
            ...
            for task in tasks:
                ...
                if not os.path.exists(temp_dir):
                    print("making temp folder at:", temp_dir)
                ...
                # resized_MNI_mask = os.path.join(temp_dir, task_bn +'path_to_resized_MNI_mask')
                ...
                # #resize the MNI Template
                # # Resize MNI mask to match the dimensions of the mean image
                # resample = fsl.FLIRT(in_file=MNI_mask, reference=mean_func, out_file=resized_MNI_mask, interp='nearestneighbour')
                # resample.run()
            ...
            try: 
                ...
                # Convert only the relevant columns to string type
                for column in values.keys():
                    tsv[column] = tsv[column].astype(str)
                tsv.loc[row_mask, values.keys()] = ["Fail" if values[column] < 90 else "Pass" for column in values.keys()]
            except:
